...
export default function BrandEditor({ brand, onChange, savedClients, onSave, onLoad, onDelete, logo, onLogoChange, references, onReferencesChange }) {
  const update = (key, value) => onChange({ ...brand, [key]: value });

  const handleColorPick = (key, hex) => {
    update(key, hex);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <label className="block text-[12px] font-bold uppercase tracking-[2px] text-slate-400">
          Colori del brand
        </label>
          ...
          <div className="space-y-2">
            ...
            {savedClients.map((c) => (
              <div key={c.id} className="flex items-center gap-2 bg-slate-800/40 rounded-lg p-2 group/item">
                {c.logo && (
                  <img src={c.logo} alt="" className="w-6 h-6 rounded object-contain bg-white/10 flex-shrink-0" />
                )}
                <button
                  onClick={() => onLoad(c)}
                ...
                <button
                  ...
                  className="opacity-0 group-hover/item:opacity-100 text-slate-500 hover:text-red-400 transition-all p-1 cursor-pointer"
                  title="Elimina"
                >
